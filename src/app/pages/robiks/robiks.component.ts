...
@Component({
  selector: "app-robiks",
  templateUrl: "./robiks.component.html",
  styleUrls: ["./robiks.component.css"]
})
export class RobiksComponent implements OnInit {
  constructor(
    public scatterService: MainService,
  ...
  addForBreed(item) {
    ...
    if (this.breedingArr.length === 2) {
      this.breedingInfo = this.scatterService.countCooldownTimes(
        this.breedingArr[0],
        this.breedingArr[1]
      );
  ...
  ngOnInit() {
    ...
    if (!this.loginEOSService.connected && this.router.url === "/robiks") {
