...
export class MainService {
  ...
  getAvailableItems() {
    return this.loginEOSService.eos.getTableRows({
      json: true,
  ...
  async getAssetById(id, owner) {
    let asset = await this.loginEOSService.eos.getTableRows({
      json: true,
      code: environment.asset,
      scope: owner,
      table: environment.tables.assets,
  ...
  countCooldownTimes(parentF, parentS) {
    ...
    let breedingTime = this.dhms(
      600 +
        (parentF.mdata.kids + parentS.mdata.kids) *
          (parentF.mdata.kids + parentS.mdata.kids <= 3 ? 3600 : 21600)
    );
    ...
    let generation = Math.round(
      (parentF.idata.gen + 1 + (parentS.idata.gen + 1)) / 2
    );
